...
class GenensConfig:
    def __init__(self, func, full, grow, term, kwargs_config):
        self.func_config = func

        self.full_config = full
        self.grow_config = grow
        self.term_config = term

        self.kwargs_config = kwargs_config

    def add_ensemble(self, ens_cls, in_arity, in_type='out', out_type='ens', evo_kwargs=None, **const_kwargs):
        name = ens_cls.__name__

        func = ensemble_func(ens_cls, **const_kwargs)
        prim = ensemble_primitive(name, in_arity, in_type, out_type)

        self.func_config[name] = func

        out_list = self.full_config.set_default(out_type, [])
...
def get_n_components(feat_size, feat_fractions=None):
    ...
    :param feat_size: Total feature count.
    :param list feat_fractions: Fractions in the interval (0.0, 1.0].
    :return: Feature counts (integer numbers).
    """
    if feat_fractions is None:
        feat_fractions = [0.01, 0.05, 0.1, 0.25, 0.5, 0.75, 1]

    return [int(feat_size * fraction) for fraction in feat_fractions]
...
def ensemble_primitive(ens_name, in_arity, in_type='out', out_type='ens'):
    ...
    :param string ens_name: Name of the ensemble.
    ...
    :param string in_type: Type of all child nodes.
    :param string out_type: Node output type.
...
def predictor_primitive(p_name):
    ...
    :param p_name: Name of the predictor.
...
def transformer_primitive(t_name):
    """
    ...
    :param t_name: Name of the transformer.
